...
async def edge_fulltext_search(
    ...
    if driver.provider == GraphProvider.FALKORDB:
        ...
        query = (
            """
            CALL db.idx.fulltext.queryNodes('FactIndex', $query)
            YIELD node AS fact_node, score
            WHERE fact_node.group_id IN $group_ids
            WITH fact_node.fact_id AS fact_id, score
            
            // Get the original edge and its connected nodes
            MATCH (n:Entity)-[e:RELATES_TO {uuid: fact_id}]->(m:Entity)
            """
            + filter_query
            + """
            WITH e, n, m, score
