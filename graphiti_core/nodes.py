...
class FactIndexNode(Node):
    """Node for fulltext indexing of facts from relationships"""
    fact_id: str = Field(description='UUID of the related RELATES_TO edge')
    text: str = Field(description='Full text of the fact')
    text_lower: str = Field(description='Lowercase version for case-insensitive search')
    keywords: str = Field(description='Extracted keywords for better search')
    
    async def save(self, driver: GraphDriver):
        """Save FactIndexNode to the graph database"""
        result = await driver.execute_query(
            """
            MERGE (n:FactIndex {uuid: $uuid})
            SET n.fact_id = $fact_id,
                n.text = $text,
                n.text_lower = $text_lower,
                n.keywords = $keywords,
                n.group_id = $group_id,
                n.name = $name,
        ...
        logger.debug(f'Saved FactIndexNode to Graph: {self.uuid}')
        
        return result
    
    @classmethod
    async def get_by_fact_id(cls, driver: GraphDriver, fact_id: str):
        """Get FactIndexNode by fact_id"""
        records, _, _ = await driver.execute_query(
            """
            MATCH (n:FactIndex {fact_id: $fact_id})
            RETURN n
            """,
            fact_id=fact_id,
            routing_='r',
        ...
        return cls(
            ...
            text_lower=node_data.get('text_lower'),
            keywords=node_data.get('keywords'),
            group_id=node_data.get('group_id'),
            name=node_data.get('name'),
            created_at=parse_db_date(node_data.get('created_at')),
        )
