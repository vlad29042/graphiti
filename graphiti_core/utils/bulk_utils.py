...
from graphiti_core.nodes import EntityNode, EpisodeType, EpisodicNode, FactIndexNode, create_entity_node_embeddings
...
async def add_nodes_and_edges_bulk_tx(
    ...
    if driver.provider == GraphProvider.FALKORDB:
        for edge in entity_edges:
            if edge.fact:  # Only create index for edges with facts
                # Extract keywords from fact (simple approach - can be improved)
                keywords = ' '.join(set(edge.fact.lower().split()))
                
                fact_index = FactIndexNode(
                    fact_id=edge.uuid,
                    text=edge.fact,
                    text_lower=edge.fact.lower(),
                    keywords=keywords,
                    group_id=edge.group_id,
                    name=f"FactIndex_{edge.uuid[:8]}"  # Short name for identification
                )
                fact_index_nodes.append(fact_index.model_dump())
    ...
    # Save FactIndex nodes for FalkorDB
    if fact_index_nodes and driver.provider == GraphProvider.FALKORDB:
        fact_index_save_query = """
        UNWIND $fact_nodes AS fact_node
        MERGE (f:FactIndex {uuid: fact_node.uuid})
        SET f = fact_node
        """
        await tx.run(fact_index_save_query, fact_nodes=fact_index_nodes)
